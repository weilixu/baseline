...
public class DistrictHeatSystem4 implements SystemType4 {
    ...
    public DistrictHeatSystem4(SystemType4 sys, EnergyPlusBuilding bldg) {
	system = sys;
	...
	HashMap<String, ArrayList<ThermalZone>> floorMap = building
		.getFloorMap();
	Set<String> floorSet = floorMap.keySet();
	Iterator<String> floorIterator = floorSet.iterator();
	while (floorIterator.hasNext()) {
		...
		if (cl.hasSpecialCharacters()) {
		    cl.replaceSpecialCharacters(floor);
		}
	...
	while (supplySystemIterator.hasNext()) {
	    ...
	    if (eo.getObjectName().equals("Coil:Heating:Electric")) {
