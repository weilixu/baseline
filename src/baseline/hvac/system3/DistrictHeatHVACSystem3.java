 ...
 * to district heating.
 * This involves two step contruction:
 * First: remove the original coil:heating:gas object and its related objects
 * Second: insert a plant side system as heating source
 * @author Weili
 *
 */
public class DistrictHeatHVACSystem3 implements SystemType3{
	...
	objectLists.get("Supply Side System").addAll(plantObjects.get("Supply Side System"));
	if(objectLists.get("Plant")==null){
	    objectLists.put("Plant", plantObjects.get("Plant"));
	}else{
	    //if correct, this line will never be executed because standard system 3 has no plant
	...
	//now we should have plant template, include supply system and plant system,
	//lets deal with supply system first, also record the necessary parts for future connection
	//with plant system
	
	ArrayList<EplusObject> supplySystem =  plantObjects.get("Supply Side System");
	...
	for(int i=0; i<building.getNumberOfZone(); i++){
		...
		if(cl.hasSpecialCharacters()){
		    cl.replaceSpecialCharacters(building.getZoneNamebyIndex(i));		    
		}
	...
	HVACSystemImplUtil.plantConnectionForDistrictHeating(plantTemp, zoneHeatCoilBranchList);
	...
	while(supplySystemIterator.hasNext()){
	    ...
	    }else if(eo.getObjectName().equals("AirLoopHVAC:Unitary:Furnace:HeatCool")){
		for(int i=0; i<eo.getSize(); i++){
		    if(eo.getKeyValuePair(i).getKey().equals("Heating Coil Object Type")){
...
//		for(int i=0; i<eo.getSize(); i++){
//		    if(eo.getKeyValuePair(i).getKey().equals("Controller List Name")){
...
//	    }
	...
	for(EplusObject eo: template){
		...
		if(!plantObjects.containsKey("Supply Side System")){
		    plantObjects.put("Supply Side System", new ArrayList<EplusObject>());
		}
		...
		if(!plantObjects.containsKey("Demand Side System")){
		    plantObjects.put("Demand Side System", new ArrayList<EplusObject>());
		}
